...
class PlayState extends MusicBeatState
{
	...
	public static var heals:Int = 10;
	public static var langoption:Int = 0;
	...
	public function addObject(object:FlxBasic) { add(object); } //i have a feeling this is even more useless
	public function removeObject(object:FlxBasic) { remove(object); }
	public function loadplayState() { LoadingState.loadAndSwitchState(new PlayState(), true); } //sunday mod source code
	...
	override public function create()
	{
		...
		{
			case 'way':
				dialogue = CoolUtil.coolTextFile(Paths.txt('way/way' + easy + language));
			case 'always':
				dialogue = CoolUtil.coolTextFile(Paths.txt('always/always' + easy + language));
			case 'no-way':
				dialogue = CoolUtil.coolTextFile(Paths.txt('no-way/no-way' + easy + language));
		}
				...
				case 'sussyBg2':
					{
						...
						var susbg:FlxSprite = new FlxSprite( -317.8, -274.85).loadGraphic(Paths.image('sussyBg/sussyBg2', 'way'));
						susbg.antialiasing = true;
						susbg.scrollFactor.set(1, 1);
						susbg.active = false;
						add(susbg);
		...
		if (!PlayStateChangeables.Optimize)
		{
			...
			if (curStage == 'MadWayBg')
			{
				var bottomMad:FlxSprite = new FlxSprite( -709, 325.7).loadGraphic(Paths.image('MadWayBg/bottom', 'way'));
				bottomMad.antialiasing = true;
				bottomMad.scrollFactor.set(0.78, 0.78);
				bottomMad.active = false;
				bottomMad.updateHitbox();
				add(bottomMad);

				var topMad:FlxSprite = new FlxSprite( -894.2, -298.05).loadGraphic(Paths.image('MadWayBg/top', 'way'));
				topMad.antialiasing = true;
				topMad.scrollFactor.set(0.8, 0.8);
				topMad.active = false;
				topMad.updateHitbox();
				add(topMad);
			}
		...
		{
			case 'power-link':
				healthBar.createFilledBar(0xFFCC2D2D, 0xFF331919);
			case 'sunshine':
				healthBar.createFilledBar(0xFFBABABA, 0xFF22BBE6);
			case 'high-school-conflict':
				healthBar.createFilledBar(0xFFB386A9, 0xFF22BBE6); //imagine simping for way
		}
		add(healthBar);
	...
	override public function update(elapsed:Float)
	{
		...
		if (FlxG.keys.justPressed.THREE)
		{
			if (iconP1.animation.curAnim.name.startsWith('bf'))
				iconP1.animation.play(SONG.player2);
			else
				iconP1.animation.play(SONG.player1);
			if (iconP2.animation.curAnim.name.startsWith('bf'))
				iconP2.animation.play(SONG.player2);
			else
				iconP2.animation.play(SONG.player1);
		}
		...
		if (FlxG.keys.justPressed.FOUR)
		{
			if (luaModchart != null)
			{
				luaModchart.die();
				luaModchart = null;
				modchartdel.scrollFactor.set();
				modchartdel.screenCenter(FlxAxes.X);
				add(modchartdel);
				modchartdel.alpha = 0;
				FlxTween.tween(modchartdel, {alpha: 1, y:modchartdel.y + 1}, 0.4, {ease: FlxEase.quartInOut});
				new FlxTimer().start(2, function (e:FlxTimer) {
					FlxTween.tween(modchartdel, {alpha: 0, y:modchartdel.y + 1.4}, 0.6, {ease: FlxEase.quartInOut});
					remove(modchartdel);
				});
			} //ig ill just leave it disable only
			// else
			// {
			// 	remove(modchartdel);
			// 	modchartyes.scrollFactor.set();
			// 	modchartyes.screenCenter(FlxAxes.X);
			// 	add(modchartyes);
			// 	modchartyes.alpha = 0;
			// 	FlxTween.tween(modchartyes, {alpha: 1, y:modchartyes.y + 1}, 0.4, {ease: FlxEase.quartInOut});
			// 	new FlxTimer().start(2, function (e:FlxTimer) {
			// 		FlxTween.tween(modchartyes, {alpha: 0, y:modchartyes.y + 1}, 0.5, {ease: FlxEase.quartInOut});
			// 		remove(modchartyes);
			// 	});
			// 	var songLowercase = StringTools.replace(PlayState.SONG.song, " ", "-").toLowerCase();
			// 	switch (songLowercase)
				...
				if (FlxG.keys.justPressed.SPACE)
				{
					switch(boyfriend.curCharacter)
					{
						case 'bf':
							...
							gf.playAnim('sad');
							FlxG.sound.play(Paths.sound('notheal'));
						case 'bf-pixel':
		...
		else
		{
			if (isStoryMode)
			{
				...
				else
				{
					...
					{
						if (curSong == 'always' && !isCutscene)
						{
							FlxG.camera.fade(FlxColor.BLACK, 1, false, function(){
								video.playMP4(Paths.video('always'), new PlayState()); 
								isCutscene = true;
							});
						}
						else if (curSong == 'no-way' && !isCutscene)
							{
								FlxG.camera.fade(FlxColor.BLACK, 1, false, function(){
									video.playMP4(Paths.video('sussus'), new PlayState()); 
									isCutscene = true;
								});
							}
						else
	...
	*/
	...
	override function stepHit()
	{
		...
		else //this code sucks but works
		{
			if (curSong == 'no-way') 
			{
				switch (curStep)
	...
	override function beatHit()
	{
		...
		function sus2() {
			...
			remove(gf);
			remove(dad);
			var susbg2:FlxSprite = new FlxSprite( -317.8, -274.85).loadGraphic(Paths.image('sussyBg/sussyBg2', 'way'));
			susbg2.antialiasing = true;
			susbg2.scrollFactor.set(1, 1);
			susbg2.active = false;
			add(susbg2);
			add(gf);
			add(dad);
